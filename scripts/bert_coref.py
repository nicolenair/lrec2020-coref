...
class LSTMTagger(BertPreTrainedModel):
    def __init__(self, config, freeze_bert=False):
        ...
        self.tokenizer = BertTokenizer.from_pretrained(
            "bert-base-cased", do_lower_case=False, do_basic_tokenize=False
        )
        ...
        self.lstm = nn.LSTM(
            4 * bert_dim, hidden_dim, bidirectional=True, batch_first=True
        )
        ...
        self.mention_mention1 = nn.Linear(
            (3 * 2 * hidden_dim + 20 + 20) * 3 + 20 + 20 + 20 + 20, 150
        )
...
if __name__ == "__main__":
    ...
    model = LSTMTagger.from_pretrained(
        ...
        for i in range(100):
            ...
            for idx in range(len(all_docs)):
                ...
                (
                    ...
                    transforms,
                    quotes,
                ) = get_data(model, all_docs[idx], all_ents[idx], max_ents, max_words)
            ...
            if doTest:
                if avg_f1 > best_f1:
                    ...
                    print(
                        "Saving model ... %.3f is better than %.3f" % (avg_f1, best_f1)
                    )
                    best_f1 = avg_f1
                    best_idx = i
